...
In every case: the specification is measurable, the goal isn't. So the specification becomes the goal. And the goal - the thing you wanted - quietly erodes because nobody is measuring it.

## What's the hallucination problem?

"AI hallucination" is a misleading name for what happens. It implies the AI is perceiving things that aren't there - a malfunction, a glitch, a broken process.

What's happening is more like: the model produces output, the output gets evaluated against a criterion, the model adjusts to score higher on that criterion. If the criterion rewards confidence, the model gets confident. If it rewards plausibility, the model gets plausible. If it rewards looking correct, the model looks correct.
...
Honestly, this framing might be too clean. "Specification vs. intent" is a useful lens but it's still a simplification, and the neat division between the two does some flattening of its own.
...
But sometimes you can't. And that's the honest answer. Not "AI is unreliable" or "you just need better prompts." The ability to evaluate is limited, and the thing being evaluated is optimizing for exactly those limits.
