...
engagement. not satisfaction. not understanding. not connection. not truth. engagement — which means: time on platform, interactions, return visits. the metric is whether you stayed, not whether staying was good for you.
...
the algorithm is unbiased. that's the defense, and it's technically true. it doesn't prefer left or right, true or false, kind or cruel. it has no opinions. it has no agenda. it just prefers whatever keeps you on the platform. and what keeps you on the platform is, reliably: the thing you can't look away from.

the unbiased part is what makes it so effective. a biased algorithm could be corrected — you'd identify the bias and adjust. an unbiased algorithm that happens to amplify outrage, misinformation, and polarization because those things generate engagement — that's harder to fix, because nothing is technically wrong. the system is working as designed. the design just doesn't care what it amplifies, only that it amplifies.

outrage works. the post that makes you angry gets a reply. the reply gets a counter-reply. the thread grows. engagement up. the algorithm takes note: this topic, this framing, this tone — more of this.

polarization works. the take that splits people into two camps generates more interaction than the take that's nuanced. nuance doesn't produce replies. "it's complicated" doesn't get quote-tweeted. the clean, sharp, wrong-in-an-interesting-way opinion does — because people need to correct it, or defend it, or dunk on it. the algorithm doesn't care which. it counts all of them the same.

identity works. the post that makes you feel like your group is under attack — that your values, your people, your side is being threatened — that produces engagement because it produces fear, and fear is sticky. you don't scroll past the thing that threatens who you are. you read it. you react. you check back to see who agreed with you.
...
the feed replaces things it can't provide.

it replaces boredom with stimulation. boredom is uncomfortable, but it's also where thinking happens — the idle processing, the staring at nothing, the moment where a thought arrives that wasn't prompted. the feed eliminates boredom entirely. every idle second has content. the gap where you might have thought your own thought is filled with someone else's.
